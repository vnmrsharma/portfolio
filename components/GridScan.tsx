...
// Type assertion helper for THREE namespace to avoid type conflicts
...
export const GridScan = ({
  ...
  const lookTarget = useRef<any>(new THREE_ANY.Vector2(0, 0));
  const tiltTarget = useRef(0);
  const yawTarget = useRef(0);

  const lookCurrent = useRef<any>(new THREE_ANY.Vector2(0, 0));
  const lookVel = useRef<any>(new THREE_ANY.Vector2(0, 0));
  ...
  useEffect(() => {
    ...
    const uniforms = {
      iResolution: {
        value: new THREE_ANY.Vector3(container.clientWidth, container.clientHeight, renderer.getPixelRatio())
      },
      iTime: { value: 0 },
      uSkew: { value: new (THREE as any).Vector2(0, 0) },
      uTilt: { value: 0 },
      uYaw: { value: 0 },
    ...
    const material = new THREE_ANY.ShaderMaterial({
      uniforms,
      vertexShader: vert,
      fragmentShader: frag,
      transparent: true,
    ...
    const camera = new THREE_ANY.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    const quad = new THREE_ANY.Mesh(new THREE_ANY.PlaneGeometry(2, 2), material);
    scene.add(quad);
    ...
    if (enablePost) {
      ...
      (bloom.blendMode as any).opacity.value = Math.max(0, bloomIntensity);
      bloomRef.current = bloom;

      const chroma = new ChromaticAberrationEffect({
        offset: new THREE_ANY.Vector2(chromaticAberration, chromaticAberration),
    ...
    const tick = () => {
      ...
      const skew: any = new THREE_ANY.Vector2(lookCurrent.current.x * skewScale, -lookCurrent.current.y * yBoost * skewScale);
      (material.uniforms as any).uSkew.value.set(skew.x, skew.y);
      (material.uniforms as any).uTilt.value = tiltCurrent.current * tiltScale;
      (material.uniforms as any).uYaw.value = THREE.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      (bloomRef.current.blendMode as any).opacity.value = Math.max(0, bloomIntensity);
      (bloomRef.current.luminanceMaterial as any).threshold = bloomThreshold;
      (bloomRef.current.luminanceMaterial as any).smoothing = bloomSmoothing;
    }
...
function srgbColor(hex: string) {
  const c = new THREE_ANY.Color(hex);
  return c.convertSRGBToLinear();
}

function smoothDampVec2(current: any, target: any, currentVelocity: any, smoothTime: number, maxSpeed: number, deltaTime: number) {
